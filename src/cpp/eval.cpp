...
/* square, square root and factorial functions are accessed when their corresponding single character
   i.e. 'A' for sq,'B' for sr, and 'C' for fact is encountered
*/
int square(int x){
    return x*x;
}
...
int command(node *cur, char c, int ptr, int mxptr, int cur_size) {
    ...
    if(c == 'B'){
        v[ptr]=square_root(v[ptr]);
        return ptr;
    }
    if(c == 'C'){
        v[ptr]=factorial(v[ptr]);
        return ptr;
    }
    if ( c == '>' ) {
        return ++ptr;
